...
def RunHRCOA(func):
    ...
    for i in range(TrialRuns):
        ...
        while curIter <= MaxIter:
            ...
            curIter += 1
            Best_list.append(min(FitPop))
        All_Best.append(Pop[np.argmin(FitPop)])
        All_Trial_Best.append(np.abs(Best_list))
    np.savetxt("./HRCOA_Data/WSN/Sol/WSN_" + str(int(DimSize/2)) + ".csv", All_Best, delimiter=",")
    np.savetxt("./HRCOA_Data/WSN/Obj/WSN_" + str(int(DimSize/2)) + ".csv", All_Trial_Best, delimiter=",")


def main(dim):
    global PopSize, Pop, DimSize, MaxFEs, MaxIter, SuiteName, LB, UB
...
if __name__ == "__main__":
    if os.path.exists('./HRCOA_Data/WSN/Obj') == False:
        os.makedirs('./HRCOA_Data/WSN/Obj')
    if os.path.exists('./HRCOA_Data/WSN/Sol') == False:
        os.makedirs('./HRCOA_Data/WSN/Sol')
    Dims = [64, 84, 108]
    for dim in Dims:
        main(dim)
